    ...
    <div className="mx-auto max-w-3xl bg-white p-8 text-[13px] text-gray-800">
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-xl bg-gray-50 p-3">
          ...
          <InfoRow label="Invoice No." value={invoice.invoice_number} bold />
          <InfoRow label="Invoice Date" value={formatDate(invoice.created_at)} />
          <InfoRow label="Customer's PO No." value="N/A" />
          <InfoRow label="P.O Date" value="N/A" />
          <InfoRow label="Quotation Ref" value={quotation?.quote_number || "—"} />
        </div>
      </div>
        ...
        <tbody>
          {(items ?? []).map((item: any, i: number) => (
            <tr key={item.id} className="border-b border-gray-100">
              ...
              <td className="py-1.5 text-right align-top text-gray-300">—</td>
      ...
      {noVat && (
        <p className="mt-3 text-xs italic text-gray-400">
          Note: We are not registered for UAE VAT. No VAT has been charged on this invoice as
        ...
        <div className="rounded-xl bg-gray-50 p-3 text-[11px] leading-relaxed text-gray-600">
          ...
          <p className="mb-2">
            ...
            {delivery?.status === "delivered" && delivery.delivery_date
              ? `Completed on ${formatDate(delivery.delivery_date)}`
              : "Pending"}
          </p>
          <p>
            <span className="font-semibold uppercase tracking-wide text-gray-500">
